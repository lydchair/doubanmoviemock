...
class SwiperList extends React.Component {
    ...
    render(){
        let list = this.props.showingList, 
            ulW = this.liW*list.length;    
            ...
            <ul ref="swiperUl" style={{width:`${ulW}px`, transform:`translate3d(${this.state.posX}px,0,0)`}}>
                {
                    list.map((item,index)=>{
                        return <li key={item.id}><Link to={`/details/${item.id}`}>
                            ...
                            <p className="item-average"><span className="average-txt">Rating:<span className="num">{item.rating.average}</span></span></p>
                        </Link></li>
                    })
                }
            </ul>
